...
\begin{frame}
  ...
  \begin{theorem}
    ...
    \begin{itemize}
    \item Algorithm 2 will eventually halt. \alert{\scriptsize proof: B[n] decrease and L is bounded}.
    \item The result we get is unique independent of the order in which the nodes are visited. \alert{\scriptsize proof: B[n] is the  maximal fix point of the set of equations}.
    \item $(\forall n \in N) H[n] \leq \bigwedge_{P \in PATH(n)}f_P(0)$. \alert{\scriptsize proof: Induction on the length of P}.
    \item If A[n] is the result of applying Algorithm 1 to $I = (G, M)$, then $A[n] \leq H[n]$. \alert{\scriptsize Let's prove this}.
    \end{itemize}
  \end{theorem}
\end{frame}

\begin{frame}
  \frametitle{Properties of Algorithm - Proof}
  Proposition: If A[n] is the result of applying Algorithm 1 to $I = (G, M)$, then $A[n] \leq H[n]$.

  \begin{proof}[Proof by induction on steps of algorithm 2]
    \only<1>{
      It's equivalent to prove following proposition:
      \begin{equation}
        (\forall p \in N) f_p(A[p]) \leq B[p] \Longleftarrow
        \begin{cases}
      ...
      \alert{Basis}. Initially $B^0[n] = 1(\text{except }n_0), A[n_0] = 0$, so we have $\forall n. f_n(A[n]) \leq B^0[n]$.
    }
    \only<2>{
      \alert{Induction Step}. From the induction hypothesis(IH) we have:
      \begin{equation}
        (\forall n \in N) B^{m-1}[n] \geq f_n(A[n]) % \text{ for all n \in N}
      \end{equation}
      ...
      \begin{equation*}
        \begin{array}{lcll}
          ...
          & = & \bigwedge_{p \in PRED(n)} f_n(f_p(A[p]))  & \quad \text{\scriptsize{(monotonicity \& IH)}} \\
          & \geq & f_n(\bigwedge_{p \in PRED(n)} f_p(A[p])) & \quad \text{\scriptsize{(monotonicity)}} \\
          & = & f_n(A[n])
        \end{array}
      \end{equation*}
    }
...
\begin{frame}
  ...
  \begin{equation}
    MOP = \bigwedge_{P \in PATH(n)}f_P(0) \geq f(x) \wedge f(y) \geq f(x \wedge y) = H[n].
  \end{equation}
