...
\begin{frame}
  ...
  \begin{block}{Proof by induction on steps of algorithm 2}
      The proposition is equivalent to the following proposition:
      \[
    ...
    \]
      ...
      \alert{Base step}. Initially $B^0[n] = 1(\text{except }n_0), A[n_0] = 0$,
      so we have for all $n \in N$, $f_n(A[n]) \leq B^0[n]$.
  \end{block}
\end{frame}
\begin{frame}
   ...
   B^m[n] & = \bigwedge_{p \in PRED(n)} f_n(B^{m-1}[p]) \\
          ...
          & \geq f_n(\bigwedge_{p \in PRED(n)} f_p(A[p])) & \text{(monotonicity)} \\
          & =  f_n(A[n])
      \end{align*}
  \end{proof}
\end{frame}
