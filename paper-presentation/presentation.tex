...
\begin{frame}[fragile]
    ...
    \begin{definition}
        A \alert{semilattice} is a set $L$ with \emph{meet} operation $\wedge$
        where the meet operation satisfies the following properties:
        \begin{align*}
...
\begin{frame}[fragile]
\frametitle{Semilattice: ordering}
\begin{definition}
    The meet $\wedge$ defines a \alert{partial order} on $L$
    \begin{align*}
...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=0.13]
        \tikzstyle{every node}+=[inner sep=0pt]
        \draw [black] (35.2,-19.2) circle (3);
        \draw (35.2,-19.2) node {$d$};
        \draw [black] (28.2,-29.1) circle (3);
        \draw (28.2,-29.1) node {$b$};
        \draw [black] (42,-29.1) circle (3);
        \draw (42,-29.1) node {$c$};
        \draw [black] (35.2,-39.2) circle (3);
        \draw (35.2,-39.2) node {$a$};
        \draw [black] (33.49,-36.73) -- (29.91,-31.57);
        \fill [black] (29.91,-31.57) -- (29.95,-32.51) -- (30.78,-31.94);
        \draw [black] (36.88,-36.71) -- (40.32,-31.59);
        \fill [black] (40.32,-31.59) -- (39.46,-31.97) -- (40.29,-32.53);
        \draw [black] (29.93,-26.65) -- (33.47,-21.65);
        \fill [black] (33.47,-21.65) -- (32.6,-22.01) -- (33.41,-22.59);
        \draw [black] (40.3,-26.63) -- (36.9,-21.67);
        \fill [black] (36.9,-21.67) -- (36.94,-22.62) -- (37.76,-22.05);
    \end{tikzpicture}
    \caption{$d > b, c > a$. However, $b \not > c$ and $c \not > b$.}
\end{figure}
\end{frame}
...
\begin{frame}[fragile]
    ...
    \begin{definition}
        ...
        to be a \emph{monotone function space} associated with $L$ if the
        following conditions are satisfied:
        \begin{enumerate}
            \item[M1] Each $f \in F$ satisfies the \alert{monotonicity condition} if for all $x, y \in L$,
                \[
                    f (x \wedge y) \leqq f(x) \wedge f(y)
                \]
            \item[M2] There exists an \alert{identity function} $i$ in $F$.
            \item[M3] $F$ is \alert{closed} under composition.
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                ...
                \item $\forall f \in F, f: L \rightarrow L$
      ...
      \begin{center}
        ...
        \only<2>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, text=red, right of=n2, node distance=0.6cm] (s2) {0};
            \node [block, below of=n1, label=130:$n_3$, xshift=0.85cm] (n3) {$C = A + B$};
            \node [value, right of=n3, node distance=0.8cm] (s3) {1};
            \node [block, below of=n3, label=100:$n_4$] (n4) {$i$};
            \node [value, right of=n4, node distance=0.5cm] (s3) {1};
        ...
        \only<3>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, text=red, left of=n1, node distance=0.6cm] (s1) {0};
            \node [block, below of=n0, label=above:$n_2$, xshift=0.8cm] (n2) {$A = 3$\\$B = 2$};
        ...
        \only<4>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, text=red, right of=n3, node distance=0.8cm] (s3) {0};
            \node [block, below of=n3, label=100:$n_4$] (n4) {$i$};
            \node [value, right of=n4, node distance=0.5cm] (s3) {1};
            % Draw edges
            \path [line] (n0) -- (n1);
        ...
        \only<6>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, text=red, right of=n4, node distance=0.5cm] (s3) {0};
            % Draw edges
            \path [line] (n0) -- (n1);
            \path [line] (n0) -- (n2);
            \path [line] (n1) -- (n3);
...
\subsection{Properties of Algorithm 1}

\begin{frame}
  \frametitle{Properties of Algorithm 1}
  \begin{theorem}
    Given an instance $I = (G, M)$ of a framework $D = (L, \wedge, F)$ as input to Algorithm 1:

    \begin{enumerate}[(i)]
    \item<1> Algorithm 1 will eventually halt. \alert{\scriptsize proof: A[n]
        decrease and L is bounded}.
    \item<2> $(\forall n \in N) A[n] \leqq \bigwedge_{P \in PATH(n)}f_P(0)$.
        \alert{\scriptsize proof: Induction on the length of P}.
    \item<3> The result we get is unique independent of the order in which the
        nodes are visited. \alert{\scriptsize proof: A[n] is the  maximal fix
        point of the set of equations}.
    \item<4> There're cases where A[n] is strictly less than MOP when the data flow graph is monotone. \alert{\scriptsize proof: illustrated in the example}.
    \end{enumerate}
  \end{theorem}
\end{frame}
...
\begin{frame}
 ...
 \begin{columns}[onlytextwidth]
    \begin{column}{0.55\textwidth}
      Visit nodes other than $n_0$ in order $v_1, v_2, ...$, for each visited node $n$, set \\
      \begin{equation*}
        B[n] = \bigwedge_{p \in PRED(n)} f_{\alert{n}}(B[p])
      \end{equation*}
      ...
      \begin{center}
        ...
        \only<2>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, left of=n1, node distance=0.6cm] (s1) {1};
            \node [block, fill=red, below of=n0, label=above:$n_2$, xshift=0.8cm] (n2) {$A = 3$\\$B = 2$};
            \node [value, text=red, right of=n2, node distance=1.2cm] (s2) {\{(A, 3), (B, 2)\}};
            \node [block, below of=n1, label=130:$n_3$, xshift=0.85cm] (n3) {$C = A + B$};
            \node [value, right of=n3, node distance=0.8cm] (s3) {1};
            \node [block, below of=n3, label=100:$n_4$] (n4) {$i$};
            \node [value, right of=n4, node distance=0.5cm] (s3) {1};
        ...
        \only<3>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, right of=n0, node distance=0.8cm] (s0) {\{(A, 1)\}};
            \node [block, fill=red, below of=n0, label=above:$n_1$, xshift=-0.8cm] (n1) {$A = 2$\\$B = 3$};
            \node [value, text=red, left of=n1, node distance=1.2cm] (s1) {\{(A, 2), (B, 3)\}};
            \node [block, below of=n0, label=above:$n_2$, xshift=0.8cm] (n2) {$A = 3$\\$B = 2$};
        ...
        \only<4>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, right of=n2, node distance=1.2cm] (s2) {\{(A, 3), (B, 2)\}};
            \node [block, fill=red, below of=n1, label=130:$n_3$, xshift=0.85cm] (n3) {$C = A + B$};
            \node [value, text=red, right of=n3, node distance=1.2cm] (s3) {\{(C, 5)\}};
            \node [block, below of=n3, label=100:$n_4$] (n4) {$i$};
            \node [value, right of=n4, node distance=0.5cm] (s3) {1};
            % Draw edges
            \path [line] (n0) -- (n1);
        ...
        \only<6>{
          \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
            ...
            \node [value, right of=n3, node distance=1.2cm] (s3) {\{(C, 5)\}};
            \node [block, fill=red, below of=n3, label=100:$n_4$] (n4) {$i$};
            \node [value, text=red, right of=n4, node distance=0.9cm] (s3) {\{(C, 5)\}};
            % Draw edges
            \path [line] (n0) -- (n1);
            \path [line] (n0) -- (n2);
            \path [line] (n1) -- (n3);
    ...
    \begin{column}{0.5\textwidth}
      \begin{center}
        \begin{tikzpicture}[node distance = 2cm, auto, every text node part/.style={align=left}]
          ...
          \node [value, right of=n0, node distance=0.6cm] (s0) {0};
          \node [block, below of=n0, label=above:$n_1$, xshift=-0.8cm] (n1) {$A = 2$\\$B = 3$};
          \node [value, left of=n1, node distance=1cm] (s1) {$\{(A, 1)\}$};
          \node [block, below of=n0, label=above:$n_2$, xshift=0.8cm] (n2) {$A = 3$\\$B = 2$};
          \node [value, right of=n2, node distance=1cm] (s2) {$\{(A, 1)\}$};
          \node [block, below of=n1, label=130:$n_3$, xshift=0.85cm] (n3) {$C = A + B$};
          \node [value, right of=n3, node distance=0.8cm] (s3) {0};
          \node [block, below of=n3, label=100:$n_4$] (n4) {$i$};
...
\begin{frame}
  \frametitle{Properties of Algorithm 2}
  \begin{theorem}
    Given an instance $I = (G, M)$ of a framework $D = (L, \wedge, F)$ as input to Algorithm 2:

    \begin{enumerate}[(i)]
    \item<1> Algorithm 2 will eventually halt. \alert{\scriptsize proof: B[n]
        decrease and L is bounded}.
    \item<2> The result we get is unique independent of the order in which the
        nodes are visited. \alert{\scriptsize proof: B[n] is the  maximal fix
        point of the set of equations}.
    \item<3> $(\forall n \in N) H[n] \leqq \bigwedge_{P \in PATH(n)}f_P(0)$.
        \alert{\scriptsize proof: Induction on the length of P}.
    \item<4-5> If A[n] is the result of applying Algorithm 1 to $I = (G, M)$, then
        $A[n] \leqq H[n]$. \only<5>{\alert{Let's prove this!}}
    \end{enumerate}
  \end{theorem}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Proof by induction on steps of algorithm 2}
      ...
      For all other node $n \in N$, $A[n] \leqq H[n]$ is equivalent to:
...
\begin{frame}
    \begin{proof}[Proof by induction on steps of algorithm 2 (cont.)]
        ...
        \[
            B^{m-1}[n] \geqq f_n(A[n])
        \]

      Then
      \begin{align*}
   B^m[n] & = \bigwedge_{p \in PRED(n)} f_n(B^{m-1}[p]) \\
          & \geqq \bigwedge_{p \in PRED(n)} f_n(f_p(A[p]))  & \text{(IH)} \\
          & \geqq f_n(\bigwedge_{p \in PRED(n)} f_p(A[p])) & \text{(monotonicity)} \\
          & =  f_n(A[n])
      \end{align*}
  \end{proof}
\end{frame}
...
\begin{frame}
    \frametitle{Algorithm 2 is not Universal}
    An infinite number of counterexamples exist. Consider the following
    constructed instance of a \alert{non-distributive} monotone
    framework:
    \[
        MOP(n) = \bigwedge_{P \in PATH(n)}f_P(0) \geqq f(x) \wedge f(y) > f(x \wedge y) = H[n].
    \]
    \begin{figure}
        \centering
...
\begin{frame}[fragile]
    ...
    \begin{block}{Intuition}
        We can transform any \emph{Modified Post Correspondence Problem} (MPCP), which is well known to be \alert{undecidable}, to a MOP problem on a monotone data flow framework.

        If there exists a general algorithm to solve the MOP problem, then we can use it to decide whether any given MPCP problem has a solution or not, which is a contradiction.
    \end{block}
\end{frame}
\begin{frame}[fragile]
  \frametitle{MPCP example}

  \begin{definition}[Modified Post Correspondence Problem]
    Given arbitrary lists A and B, of k strings each in $\{0, 1\}^+$, say
    \begin{equation*}
      A = w_1, w_2, \ldots, w_k \quad B = z_1, z_2, \ldots, z_k
    \end{equation*}
    does there exist a sequence of integers $i_1, i_2, \ldots, i_r$ such at
    \begin{equation*}
      w_1w_{i_1}w_{i_2} \ldots w_{i_r} = z_1z_{i_1}z_{i_2} \ldots z_{i_r}
    \end{equation*}
  \end{definition}

  \begin{columns}[onlytextwidth]
    \begin{column}{0.65\textwidth}
      This example has a solution: \alert{3, 2, 2, 4}.
      \begin{align*}
        w_1w_3w_2w_2w_4 & = \text{11,0111,1,1,10} = 1101111110 \\
        x_1x_3x_2x_2x_4 & = \text{1,10,111,111,0} = 1101111110
      \end{align*}
    \end{column}
...
\begin{frame}[fragile]
  ...
  \begin{columns}[onlytextwidth]
    \begin{column}{0.5\textwidth}
      Given
      \begin{itemize}
      \item L = 0, \$, and all strings of integers beginning with 1
      \item $x \wedge y = 0 \text{ if } x \neq y$
      \item $h(x) = 1$(string 1)
      \item $f_i(0)=0, f_i(\$)=\$, f_i(x)=\text{x i}$
      \item $g(0)=0, g(\$)=\$$
      \item $g(x) = 0 \text{ if x is a solution}$
      \item $g(x) = \$ \text{ otherwise}$
      \end{itemize}
      \alert{We have $MOP(*) = 0$ if there's a solution, otherwise $MOP(*)=\$$}.
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \includegraphics[width=1\textwidth]{img/fig4.png}
        \caption{The solution to a MPCP would be one path from $n_0$ to $n$}
      \end{figure}
    \end{column}
  \end{columns}
\end{frame}
