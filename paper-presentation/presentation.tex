...
\begin{frame}[fragile]
    ...
    \begin{block}{Intuition}
        The \emph{Modified Post Correspondence Problem} (MPCP), which is well known to be \alert{undecidable}, can be reduced to the \emph{MOP problem}.
        This is accomplished by modeling the MPCP problem as a monotone data flow framework.
    \end{block}
\end{frame}
\begin{frame}[fragile]
...
\begin{frame}[fragile]
  \frametitle{Reducing MPCP to MOP}
  \begin{columns}[onlytextwidth]
    \begin{column}{0.5\textwidth}
      Define
      \begin{itemize}
      ...
      \item $h(x) =$ ``1''
      \item $f_i(0)=0, f_i(\$)=\$$, $f_i(x)= concat(x, i)}$
      \item $g(0)=0, g(\$)=\$$
