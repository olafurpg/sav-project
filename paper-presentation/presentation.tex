...
\begin{frame}
  \frametitle{Properties of Algorithm 2 - Theorem 5}
  \begin{theorem}
...
\begin{frame}
  ...
  \begin{block}{Proof by induction on steps of algorithm 2}
      For node $n_0$, the property trivially holds because for any $e \in L$, $A[n_0] = 0 \leqq e$.
...
\begin{frame}
   ...
   B^m[n] & = \bigwedge_{p \in PRED(n)} f_n(B^{m-1}[p]) \\
          & \leqq \bigwedge_{p \in PRED(n)} f_n(f_p(A[p]))  & \text{(IH)} \\
...
\begin{frame}
    ...
    framework:
    \[
        MOP = \bigwedge_{P \in PATH(n)}f_P(0) \geqq f(x) \wedge f(y) > f(x \wedge y) = H[n].
    \]
    \begin{figure}
        \centering
...
\end{frame}
\begin{frame}[fragile]
    \frametitle{WIP - MPCP example}
    This following MPCP instance has a solution: 3, 2, 2, 4: $w_1w_3w_2w_2w_4 =
    x_1x_3x_2x_2x_4$ = 1101111110
    \begin{table}
        \begin{tabular}{l|cc}
            % & A     & B \\
            $i$ & $w_i$ & $x_i$ \\
...
\begin{frame}[fragile]
    ...
    \begin{figure}
        ...
        \caption{One path is a solution to MPCP if it exists}
    \end{figure}
\end{frame}
\end{document}
